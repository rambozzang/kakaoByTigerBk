...
public class multiThreadTest01 {
	...
	@Order(2)
	@Test
	@DisplayName("Case01 2번째 => 600원 (10원) 취소 실패 5건 동시 호출시 1건만 성공 테스트.")
	public void testCase02() throws Exception {
		System.out.println("testCase02() 시작 " + LocalTime.now());
		// input 값
		Long cardAmount = 600L;
		Long cardVat = 10L;

		CancelInVO cancelInVO = CancelInVO.builder().dealKey(dealKey).bizType(bizTypeCancel).cardVat(10L)
				.cardAmount(600L).build();
		// when
		// 쓰레드 5개 600(10) 원 출금 서비스 동시 호출
		for (int i = 0; i < 5; i++) {
